...
int main() {
  ...
  std::string filePath = "/home/lyt/develop/ffmpeg_learn/video/juren-5s.mp4";  // 文件地址
  ...
  do {
    ...
    // 会将pFrameRGB的数据按RGB格式自动"关联"到buffer
    // 即nv12Frame中的数据改变了 out_buffer中的数据也会相应的改变
      ...
      if (pkt->stream_index == videoStreamIndex) {
        if (avcodec_send_packet(codecCtx, pkt) == 0) {
          while (avcodec_receive_frame(codecCtx, yuvFrame) == 0) {
            sws_scale(img_ctx, (const uint8_t *const *)yuvFrame->data, yuvFrame->linesize, 0, h,
